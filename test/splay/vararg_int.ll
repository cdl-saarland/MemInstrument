; RUN: %opt %loadlibs -mem2reg -meminstrument -mi-config=splay -debug-only=VarArgImpl %s -S > %t0.ll 2> %t2.stats
; RUN: %not fgrep "The # of inbounds checks inserted" %t2.stats
; RUN: %clink -ldl -l:libsplay.a -o %t1 %t0.ll
; RUN: %t1
...
entry:
  tail call void (i32, ...) @printargs(i32 3, i32 1, i32 2)
  ret i32 0
}
