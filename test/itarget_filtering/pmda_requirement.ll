; RUN: %opt %loadlibs -meminstrument -mi-mode=gatheritargets -debug-only meminstrument-itargetprovider %s -S > /dev/null 2> %t1.log
; RUN: %opt %loadlibs -checkoptimizer -meminstrument -mi-mode=filteritargets -debug-only meminstrument-itargetfilter %s -S > /dev/null 2> %t2.log
