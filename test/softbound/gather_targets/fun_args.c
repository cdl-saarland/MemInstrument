...
// CHECK: dereference check with constant size 8B for q at entry::[{{.*}},0]
// CHECK-NEXT: dereference check with constant size 4B{{.*}}at entry::[{{.*}},1]
// CHECK-NEXT: dereference check with constant size 4B for p at entry::[{{.*}},3]

// CHECK: value invariant for a at entry::[store,2]
// CHECK-NEXT: dereference check with constant size 8B for b at entry::[store,2]
