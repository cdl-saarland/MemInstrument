; RUN: %opt %loadlibs -meminstrument -S -mi-config=softbound -mi-mode=genchecks -debug-only=meminstrument %s 2>&1 | %fileCheck %s

; CHECK: MemInstrumentPass: generating checks
